...
def set_motor_velocities():
    global velocity
    for i in range(16):
        error = abs(pos_now[i] - pos_drive[i])
        velocity[i] = int(error * 1.2 + 50)
        velocity[i] = max(min(velocity[i], 1900), 0)
    print(velocity)
...
def main():
    ...
    # 低通滤波器，平滑角度数据
    angle_filter = LowPassFilter(alpha=0.5)
    pos_filter = LowPassFilter_(alpha=0.5)

    # 电机控制参数
    # velocity = [0] * 16        # 每个电机的速度
    # velocity[15] = 800
    # velocity[0] = 1100
    # velocity[1] = 1100

    global pos_cal, pos_now, pos_drive, current, velocity
    ...
    try:
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                # 关节角度
                angles, wrist = calc_hand_angles(landmarks, axes)
                ...
                pos_cal[2] = 0.8 * j[2] + 0.2 * j[0]    # 大拇指IP关节，等于IP 与 MCP_flex加权平均
                pos_cal[5] = 0.8 * j[5] + 0.2 * j[6]    # 食指PIP关节，等于PIP 与 DIP加权平均
                pos_cal[8] = 0.8 * j[9] + 0.2 * j[10]   # 中指PIP关节，等于PIP 与 DIP加权平均
                pos_cal[11] = 0.8 * j[13] + 0.2 * j[14] # 无名指PIP关节，等于PIP 与 DIP加权平均
                pos_cal[14] = 0.8 * j[17] + 0.2 * j[18] # 小拇指PIP关节，等于PIP 与 DIP加权平均

                # 大拇指
                j[0] -= j[1] * 0.36
                pos_cal[1]  =  j[0] + j[1] * 0.2  # 大拇指 MCP 关节 大拇指内侧电机
                pos_cal[0]  =  j[0]   # 大拇指 MCP 关节 大拇指外侧电机
                pos_cal[15]  = j[1]   # 手腕关节 
                ...
                pos_cal[3]  = 0.7 * j[3] + 0.3 * j[4] if j[4] > 0 else j[3]  # 食指 MCP 关节 远大拇指电机
                pos_cal[4]  = 0.6 * j[3] - 0.4 * j[4] if j[4] < 0 else j[3]  # 食指 MCP 关节 近大拇指电机
                ...
                pos_cal[6]  = 0.65 * j[7] - 0.35 * j[8] if j[8] < 0 else j[7]  # 中指 MCP 关节 远大拇指电机
                pos_cal[7]  = 0.6 * j[7] + 0.4 * j[8] if j[8] > 0 else j[7]  # 中指 MCP 关节 近大拇指电机


                # 无名指
                j[11] -= (abs(j[8]) * 0.25)
                if j[11] > 0.4: j[12] = 0.0
                pos_cal[9]  = 0.75 * j[11] - 0.25 * j[12] if j[12] < 0 else j[11]  # 无名指 MCP 关节 远大拇指电机
                pos_cal[10] = 0.75 * j[11] + 0.25 * j[12] if j[12] > 0 else j[11]  # 无名指 MCP 关节 近大拇指电机
                

                # 小拇指
                if j[15] > 0.6:j[16] = 0.0
                j[15] -= (abs(j[16]) * 0.65 - 0.22)
                pos_cal[12] = 0.5 * j[15] - 0.5 * j[16] if j[16] < 0 else j[15]  # 小拇指 MCP 关节 远大拇指电机
                pos_cal[13] = 0.5 * j[15] + 0.5 * j[16] if j[16] > 0 else j[15]  # 小拇指 MCP 关节 近大拇指电机
                if j[15] >= 1:pos_cal[12] = pos_cal[13] = 1

                pos_cal = [max(0.0, min(1.0, x)) for x in pos_cal]  # 限幅到 [0, 1]
                

                pos_drive = pos_filter.filter(pos_cal)
                pos_drive = list(map(lambda x: int(x * 4095), pos_drive))

                send_predefined_action(velocity, pos_drive, current)
                get_motor_positions()  # 读取电机反馈
