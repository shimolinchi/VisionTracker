            ...
            while True:
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks in results.multi_hand_landmarks:
                        ...
                        kps_pixel = {
                            name: np.array([lm.x * color_w, lm.y * color_h, lm.z])  # 转为 NumPy 数组
                            for name, lm in zip(
                        ...
                        try:
                            palm_edge1_model = np.linalg.norm(kps_pixel["INDEX_FINGER_DIP"] - kps_pixel["INDEX_FINGER_PIP"])
                            # print(palm_edge1_model)
                            print(f'{kps_pixel["INDEX_FINGER_DIP"][2]} \t {kps_pixel["INDEX_FINGER_PIP"][2]}')
